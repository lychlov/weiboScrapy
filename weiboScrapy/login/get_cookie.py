...
import logging
# from weiboScrapy.login.yundama import identify

IDENTIFY = 1  # 验证码输入方式:        1:看截图aa.png，手动输入     2:云打码
COOKIE_GETWAY = 1  # 0 代表从https://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.18) 获取cookie   # 1
# 代表从https://weibo.cn/login/获取Cookie
...
def update_cookie(accountText, rconn, spiderName):
    """ 更新一个账号的Cookie """
    ...
    if len(cookie) > 0:
        ...
        remove_cookie(accountText, rconn, spiderName)


def remove_cookie(accountText, rconn, spiderName):
    """ 删除某个账号的Cookie """
